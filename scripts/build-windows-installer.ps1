...
$tempRoot = Join-Path $env:TEMP "eoac-installer-build"
$payload = Join-Path $tempRoot "payload"
$work = Join-Path $tempRoot "iexpress"
$setup = Join-Path $tempRoot "EarthOnlineAchievementCenter-Setup.exe"
$finalSetup = Join-Path $dist "EarthOnlineAchievementCenter-Setup.exe"
...
Copy-Item -LiteralPath (Join-Path $root "launch-earth-online-achievement-center.ps1") -Destination $payload
...
FinishMessage=
TargetName=$setupEsc
FriendlyName=EarthOnlineAchievementCenter
